# Chapter 2: Getting Started with Claude Code {#_chapter_2:_getting_started_with_claude_code} {#ch02-getting-started-with-claude-code}

[]{.index term="Claude Code"} []{.index term="agent"}
...
## The Agent Mindset {#_the_agent_mindset}

[]{.index term="agent mindset"}
...
## Installation and Setup {#_installation_and_setup}
...
### Installation Steps {#_installation_steps}
...
### Your First CLAUDE.md {#_your_first_claude_md}

[]{.index term="CLAUDE.md"}
...
Claude Code will: 1. Read your CLAUDE.md to understand project context 2. Use the Glob tool to list files in src/ 3. Return a structured response

### Interactive Session {#_interactive_session}
...
### Walkthrough: Building a CLI Tool {#_walkthrough:_building_a_cli_tool}
...
Success criteria:
- Running 'npm run csv-reader data/test.csv' prints
...
## The Tool Ecosystem {#_the_tool_ecosystem}

[]{.index term="tool use"}

Claude Code has six core tools. Understanding when to use each one makes your prompts more effective.

### Read (Understand Existing Code) {#_read_(understand_existing_code)}
...
### Write (Create New Files) {#_write_(create_new_files)}
...
### Edit (Modify Existing Files) {#_edit_(modify_existing_files)}
...
### Glob (Find Files by Pattern) {#_glob_(find_files_by_pattern)}
...
### Grep (Search File Contents) {#_grep_(search_file_contents)}
...
### Bash (Execute Commands) {#_bash_(execute_commands)}
...
### The Feedback Loop {#_the_feedback_loop}
...
## Basic Prompting Patterns {#_basic_prompting_patterns}

Four patterns produce consistently good results.

### Pattern 1: Context + Goal + Success Criteria {#_pattern_1:_context_+_goal_+_success_criteria}
...
### Pattern 2: Reference Existing Code {#_pattern_2:_reference_existing_code}
...
### Pattern 3: Tests as Specification {#_pattern_3:_tests_as_specification}
...
### Pattern 4: Exploration Before Implementation {#_pattern_4:_exploration_before_implementation}
...
### Pattern 5: Incremental Development {#_pattern_5:_incremental_development}
...
**Increment sizing guide**: - Single interface or type: 5 to 20 lines - Single utility function: 10 to 30 lines - Single service method: 20 to 50 lines - Single API endpoint: 20 to 60 lines
...
## Claude Code vs Cursor vs ChatGPT {#_claude_code_vs_cursor_vs_chatgpt}
...
:   Best for multi-file workflows and automation. Full repo context via CLAUDE.md. Speed: \~30s per turn. Runs tests, builds, linters. Token-based cost.

**Cursor**

:   Best for quick edits and real-time coding. Context from open files plus search. Speed: \<1s inline. Limited verification. Subscription cost.

**ChatGPT**

:   Best for concepts and brainstorming. Paste-based, limited codebase context. Speed: \~5s API. No verification. Subscription cost.

endif

:   **Use Claude Code when:** - Task spans 3+ files - You need to run tests or builds - Full codebase context is essential - You are building automation

**Use Cursor when:** - You are actively coding - Task is small and visible on screen - You want fastest iteration

**Use ChatGPT when:** - Explaining a concept - Brainstorming architecture - No codebase context needed

## The Two-Mode Mental Model {#_the_two_mode_mental_model}

The single most important pattern for productive agent work is separating exploration from implementation.

### Why Two Modes? {#_why_two_modes?}
...
### Exploration Mode {#_exploration_mode}

Goal: Build understanding before writing code.

Key questions: - "How is \[feature\] currently implemented?" - "What patterns should I follow?" - "Show me an example."

Example workflow:

``` bash
claude -p "How does error handling work in this codebase? \
  Show me 3 examples."
# Greps for error patterns, shows Result<T, E> usage

claude -p "Should I use Result<T, E> or exceptions for \
  the new payment service?"
# Claude Code analyzes codebase, shows you chose
# Result<T, E>, explains why
```

Time invested: 5 minutes. Understanding gained: enough to implement correctly.

### Implementation Mode {#_implementation_mode}
...
``` bash
claude -p "Create PaymentService using these patterns \
  from exploration:
...
### When to Use Each {#_when_to_use_each}
...
## Common Pitfalls for Newcomers {#_common_pitfalls_for_newcomers}
...
### Pitfall 1: Oversized Tasks {#_pitfall_1:_oversized_tasks}
...
### Pitfall 2: Skipping Exploration {#_pitfall_2:_skipping_exploration}
...
### Pitfall 3: Long Conversations Without Restart {#_pitfall_3:_long_conversations_without_restart}
...
### Pitfall 4: Bloated CLAUDE.md Files {#_pitfall_4:_bloated_claude_md_files}
...
### Pitfall 5: Manual Review Instead of Verification {#_pitfall_5:_manual_review_instead_of_verification}
...
### The Common Thread {#_the_common_thread}
...
### Exercise 1: Exploration Practice {#_exercise_1:_exploration_practice}
...
### Exercise 2: Prompt Quality Comparison {#_exercise_2:_prompt_quality_comparison}
...
### Exercise 3: Full Explore + Implement Workflow {#_exercise_3:_full_explore_+_implement_workflow}

Pick a feature to add to your project.

**Part A: Explore (10 min)** 1. Ask 3 to 5 questions about existing patterns 2. Document what you discover 3. Identify where your feature fits

**Part B: Implement (15 min)** 1. Write a detailed prompt based on exploration 2. Run it 3. Verify with tests and linter

**Part C: Reflect (5 min)** 1. How many iterations would this have taken without exploration? 2. What did exploration teach you?
...
The next chapter covers prompting fundamentals in depth. You will learn information theory concepts that explain why some prompts work better than others, and you will develop techniques for maximizing the value per token. See [Chapter 3: Prompting Fundamentals](#_chapter_3_prompting_fundamentals){.cross-reference}.
...
*Related chapters:* - [Chapter 1: The Compound Systems Engineer](#_chapter_1_the_compound_systems_engineer){.cross-reference} for the meta-engineering philosophy - [Chapter 4: Writing Your First CLAUDE.md](#_chapter_4_writing_your_first_claude_md){.cross-reference} for deep dives on project context files - [Chapter 6: The Verification Ladder](#_chapter_6_the_verification_ladder){.cross-reference} for test-driven agent workflows
