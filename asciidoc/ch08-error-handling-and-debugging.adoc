...
[[fig-debugging-hierarchy]]
.The Debugging Hierarchy: Where to look first when diagnosing AI agent errors
image::ch08-debugging-hierarchy.png[Debugging Hierarchy,width=600,align=center]
...
[arabic]
. *Generate frequency report*. Sort errors by occurrence count.
. *Implement prevention* for high-frequency errors (5{plus} occurrences):
* Add ESLint rules
* Create type guards
* Add CI checks
. *Update documentation*. Add prevention strategies.
. *Measure impact*. Track reduction over time.
...
After 3{plus} failed attempts at solving a problem, context rot sets in. The conversation accumulates failed implementations, error messages, and dead-end approaches. The AI gets stuck suggesting variations of things that already failed.

This is when you need clean slate recovery.

==== Recognizing the Pattern

You need a fresh start when: - Same approach repeated with variations ("`try JWT… JWT {plus} X… JWT {plus} Y…`") - Quality declining (later suggestions worse than earlier ones) - Circular references ("`let’s go back to approach 1…`") - The "`stuck`" feeling

==== The Clean Slate Process

*Step 1*: Recognize the pattern (3{plus} failed attempts)
...
[arabic]
. *Reduce task complexity*: Fewer steps per task means fewer failure points
. *Add pre-action validation*: Check constraints before attempting
. *Add post-action verification*: Confirm the outcome, not just the response
. *Implement retry with learning*: Adapt approach based on failure reason
. *Use fresh context*: The RALPH loop pattern clears context rot between tasks
...
[[fig-ratchet-effect-flow]]
.The Ratchet Effect: Commit on success locks in progress, fix on failure before proceeding
image::ch08-ratchet-effect-flow.png[Ratchet Effect Flow,width=600,align=center]
...
[arabic]
. *Five-point framework* classifies every error into Context, Model, Rules, Testing, or Quality Gate problems
. *Context debugging* solves 60% of issues by adding missing information
. *ERRORS.md* creates persistent memory so mistakes don’t repeat
. *Flaky test diagnosis* identifies and fixes intermittent failures
. *Clean slate recovery* escapes broken trajectories after 3{plus} failed attempts
. *Circuit breakers* prevent cascading failures by stopping after consecutive failures
. *Multi-layer timeout protection* caps costs at job, request, input, and budget levels
. *Recovery patterns* use checkpoint commits and the four-turn framework for long-running agent reliability
. *Learning loops* encode every problem into permanent prevention
...
NOTE: *Companion Code*: All 7 code examples for this chapter are available at https://github.com/Just-Understanding-Data-Ltd/compound-engineering-book/tree/main/examples/ch08[examples/ch08/]

_Related chapters:_

* *<<_chapter_6_the_verification_ladder,Chapter 6: The Verification Ladder>>* for the verification patterns that catch errors early
* *<<_chapter_7_quality_gates_that_compound,Chapter 7: Quality Gates That Compound>>* for quality gates that automate prevention
* *<<_chapter_9_context_engineering_deep_dive,Chapter 9: Context Engineering Deep Dive>>* for context engineering principles that improve AI output
* *<<_chapter_10_the_ralph_loop,Chapter 10: The RALPH Loop>>* for error recovery in long-running agent workflows
* *<<_chapter_15_model_strategy_and_cost_optimization,Chapter 15: Model Strategy & Cost Optimization>>* for cost optimization and budget protection strategies
