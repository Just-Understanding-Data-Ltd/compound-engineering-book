...
After 3 failed attempts (15{plus} minutes), you’ve likely spent more time than starting fresh would take. The math favors clean breaks:
...
ifndef::backend-pdf[]
*Continue broken trajectory*:: ~40 min, ~40K tokens, ~30% success rate.

*Clean slate recovery*:: ~25 min, ~20K tokens, ~80% success rate.
endif::[]
...
.The RALPH Loop: Four-phase autonomous iteration cycle
image::ch10-ralph-loop.png[RALPH Loop Cycle,width=600,align=center]
...
[[fig-ralph-cycle-time]]
.RALPH Cycle Time: Only 20% of each iteration is actual coding
image::ch10-ralph-cycle-time.png[RALPH Cycle Time,width=500,align=center]
...
- Common error: forgetting null handling  <3>
...
|Skill emphasis |Code production |Architecture {plus} verification
...
[[fig-multi-agent-coordination]]
.Multi-Agent Coordination: Human architect directs parallel agents sharing Git memory
image::ch10-multi-agent-coordination.png[Multi-Agent Coordination,width=600,align=center]
...
. *Comprehensive test suite* (80%{plus} coverage)
. *Automated quality gates* that must pass before any commit
...
* The "`stuck`" feeling after 3{plus} attempts

The solution is clean slate recovery:

[arabic]
. *Terminate* the current session
. *Document* what failed and why (root cause, not symptoms)
. *Start fresh* session with clean context
. *Frame with constraints* to avoid repeating failures
...
*Setup:* 1. Create TASKS.md with 5-10 well-sized tasks 2. Create AGENTS.md with initial knowledge (tech stack, patterns) 3. Set up basic quality gates (type-check {plus} tests)
...
*Success criteria:* - Script runs without manual intervention - Quality gates pass for 80%{plus} of iterations - AGENTS.md has grown with learnings - Learnings from iteration 1-2 prevented mistakes in iteration 3
...
[arabic]
. *Fresh context per task* prevents negative accumulation
. *Four-phase cycle* (Plan 40%, Work 20%, Review 40%, Compound critical) structures each iteration
. *Three-layer memory* (git, AGENTS.md, TASKS.md) replaces conversation history
. *Task sizing discipline* ensures tasks fit single context windows
. *Clean slate recovery* escapes stuck trajectories
...
NOTE: *Companion Code*: All 5 code examples for this chapter are available at https://github.com/Just-Understanding-Data-Ltd/compound-engineering-book/tree/main/examples/ch10[examples/ch10/]

_Related chapters:_

* *<<_chapter_4_writing_your_first_claude_md,Chapter 4: Writing Your First CLAUDE.md>>* for the documentation layer in depth
* *<<_chapter_7_quality_gates_that_compound,Chapter 7: Quality Gates That Compound>>* for the verification mechanisms
* *<<_chapter_8_error_handling_and_debugging,Chapter 8: Error Handling & Debugging>>* for the clean slate recovery pattern
* *<<_chapter_9_context_engineering_deep_dive,Chapter 9: Context Engineering Deep Dive>>* for context degradation theory
* *<<_chapter_11_sub_agent_architecture,Chapter 11: Sub-Agent Architecture>>* for multi-agent coordination
* *<<_chapter_13_building_the_harness,Chapter 13: Building the Harness>>* for the complete orchestration system
* *<<_chapter_16_building_autonomous_systems,Chapter 16: Building Autonomous Systems>>* for the full case study of RALPH in production
