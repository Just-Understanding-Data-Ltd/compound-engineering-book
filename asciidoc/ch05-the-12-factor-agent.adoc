...
[[fig-reliability-chasm]]
.The Reliability Chasm: How 95% per-action success compounds to failure
image::ch05-reliability-chasm.png[Reliability Chasm,width=600,align=center]
...
[arabic]
. Define an `Event` union type with at least these variants: `user_input`, `tool_call`, `tool_result`, `error`, `human_response`
...
[arabic]
. Create a tool classification function that identifies tools as `immediate`, `requires_approval`, or `terminal`
