...
[[fig-state-space-reduction]]
.State Space Reduction: Each gate intersection shrinks the set of valid programs
image::ch07-state-space-reduction.png[State Space Reduction,width=600,align=center]
...
[[fig-quality-gates-filters]]
.Quality Gates as Information Filters: Progressive state space reduction
image::ch07-quality-gates-filters.png[Quality Gates as Filters,width=700,align=center]
...
Linear thinking would predict: 10% {plus} 15% {plus} 20% {plus} 15% {plus} 20% {plus} 25% = 105%

Compounding reality gives you: 165%

The bonus from compounding: 60% additional improvement.

[[fig-compounding-effect]]
.The Compounding Effect: Quality gates multiply, not add
image::ch07-compounding-effect.png[Compounding Effect,width=700,align=center]
...
. You manually run `npm run lint`
. Find 5 linting errors
. Ask Claude to fix them
. Manually run `tsc --noEmit`
. Find 3 type errors
. Ask Claude to fix those
. Manually run `npm test`
. Find 2 test failures
. Repeat…
...
[arabic]
. Claude writes a file (e.g., `src/utils/auth.ts`)
. Hook runs: `npx eslint src/utils/auth.ts --fix`
. If linting fails, Claude sees the error immediately
...
When a hook fails, press *Ctrl{plus}O* to view the complete error output, see which hook failed, get the exact command that was run, and see stack traces and assertion failures.
...
[[fig-manual-vs-auto-verification]]
.Manual vs Automated Verification: Hooks deliver 60-70% time savings
image::ch07-manual-vs-auto-verification.png[Manual vs Auto Verification,width=600,align=center]
...
[arabic]
. LLM generates code
. Type checker fails. LLM reads error, adds types.
. Linter fails. LLM reads error, fixes patterns.
. Tests fail. LLM reads assertion, fixes logic.
. CI fails. LLM reads stack trace, fixes deployment issue.
...
[arabic]
. *Types* (foundation): TypeScript, interfaces, contracts
. *Tests* (validation): Unit and integration tests
. *Linting* (consistency): ESLint, code style, patterns
. *CI/CD* (automation): GitHub Actions, automated verification
. *DDD* (architecture): Domain-driven design, bounded contexts
. *CLAUDE.md* (context): Hierarchical documentation for AI agents

==== Implementation Order

Week 1: Types {plus} Hooks (type checking on every edit) Week 2: Tests {plus} Hooks (tests run on every file write) Week 3: Linting {plus} CLAUDE.md (consistent patterns, documented for LLMs) Week 4: CI/CD automation (GitHub Actions for lint, test, review) Week 5{plus}: Refine rules, add DDD patterns, evolve CLAUDE.md
...
*Missing Gates Create Gaps*: Have types {plus} tests but no linting. Patterns diverge. CLAUDE.md cannot catch everything. Solution: fill all gaps before adding new gates. Compounding requires completeness.
...
[width="100%",cols="39%,46%,15%",options="header",]
|===
...
. Write `post-write.json` with ESLint command
. Create a TypeScript file with intentional style violations
. Have Claude write a function in that file
. Observe the hook running and auto-fixing violations
. Press Ctrl{plus}O to see full error output
...
[arabic]
. Identify a project with quality gates
. Estimate individual gate improvements based on failure rate reduction
. Calculate linear expectation: sum all improvements
. Calculate multiplicative reality: product all (1 {plus} improvement) factors
. Identify the compounding bonus
...
. Day 0: Install and configure linting (30 min)
. Days 1-5: Write code normally, track how often hooks block commits
. Calculate: setup time {plus} per-commit overhead
. Compare to estimated cleanup cost if linting added at month 3
...
Claude Code hooks automate gates, turning manual verification into real-time feedback loops. Hooks catch 85%{plus} of errors before CI/CD runs, with immediate feedback (2-5 seconds) versus delayed CI feedback (minutes to hours).

Early linting prevents technical debt ratcheting. 30 minutes of setup on day zero saves 60{plus} hours of cleanup later. The ROI is 30x or greater.
...
Partial stacks underperform by 50%{plus} compared to full stacks. Missing any gate creates compounding gaps. Add gates in batches to capture compounding effects sooner.
...
NOTE: *Companion Code*: All 3 code examples for this chapter are available at https://github.com/Just-Understanding-Data-Ltd/compound-engineering-book/tree/main/examples/ch07[examples/ch07/]

_Related chapters:_

* *<<_chapter_6_the_verification_ladder,Chapter 6: The Verification Ladder>>* for the hierarchy of verification methods that gates automate
* *<<_chapter_8_error_handling_and_debugging,Chapter 8: Error Handling & Debugging>>* for handling gate failures gracefully
* *<<_chapter_4_writing_your_first_claude_md,Chapter 4: Writing Your First CLAUDE.md>>* for the CLAUDE.md gate that ties everything together
