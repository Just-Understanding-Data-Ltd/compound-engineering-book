...
.Signal Processing Model: How harness layers filter noise from intent to output
image::ch13-signal-processing.png[Signal Processing Model,width=700,align=center]
...
[[fig-harness-architecture]]
.Harness Architecture: Four layers of control building on the raw LLM
image::ch13-harness-architecture.png[Harness Architecture,width=600,align=center]
...
async function swarmImplementation(spec: Spec) {
  const tasks = breakdownSpec(spec);  // <1>

  // Launch agents in parallel
  const results = await Promise.all(  // <2>
    tasks.map((task) =>
      spawnAgent({
        prompt: task.prompt,
        scope: task.files,  // <3>
  ...
  jobs: [
    {
      ...
      command: 'npm update && npm test',  // <2>
      onSuccess: 'create-pr',  // <3>
      onFailure: 'notify-slack',
    },
    {
      name: 'security-audit',
      command: 'npm audit --audit-level=moderate',
      onFailure: 'create-issue',  // <4>
    },
  ],
};
----
...
// PDF: Full table
ifdef::backend-pdf[]
[width="100%",cols="17%,24%,32%,27%",options="header",]
|===
...
ifndef::backend-pdf[]
*Session tier*:: In-memory storage. Lost on termination. Use for quick sub-agent tasks.

*File-based tier*:: TASKS.md, progress.txt. Survives process boundaries. Use for RALPH loop iterations.

*Event-sourced tier*:: Append-only event log. Full history reconstruction. Use for production agents with audit needs.
endif::[]
...
[arabic]
. *Understand State*: Verify context and requirements before acting
. *Decide Action*: Choose the appropriate response based on understanding
. *Execute*: Perform the task
. *Verify Outcome*: Confirm the action actually succeeded
...
[[fig-closed-loop-optimization]]
.Closed-Loop Optimization: Telemetry-driven automatic fixing until constraints satisfied
image::ch13-closed-loop-optimization.png[Closed-Loop Optimization,width=600,align=center]
...
. Infer root causes using profiler output and traces
. Agent proposes targeted refactoring
. Apply patch and re-run tests plus load test
. Score against constraints
. Loop until pass or escalate to human

Here is a concrete example. The constraint `heap.retained++_++growth++_++slope ++>++ 0` is violated. The agent analyzes heap snapshots and finds an event list that grows without bound:
...
Re-run verification confirms `heap.retained++_++growth++_++slope = 0`. Constraint satisfied.
...
// PDF: Full table
ifdef::backend-pdf[]
[width="100%",cols="18%,26%,35%,21%",options="header",]
|===
|Layer |Protection |Example Limit |Catches
|1. Job timeout |GitHub Actions `timeout-minutes` |15 minutes |Infinite loops
|2. Request tokens |API `max++_++tokens` parameter |4,096 tokens |Verbose responses
...
// EPUB: Definition list
ifndef::backend-pdf[]
*Layer 1 - Job timeout*:: GitHub Actions `timeout-minutes`. Limit: 15 minutes. Catches infinite loops.

*Layer 2 - Request tokens*:: API `max_tokens` parameter. Limit: 4,096 tokens. Catches verbose responses.

*Layer 3 - Input size*:: File count, lines per file. Limit: 50 files, 500 lines each. Catches file explosions.

*Layer 4 - Budget alerts*:: Daily and monthly caps. Limit: $10/day, $100/month. Catches sustained overuse.

*Layer 5 - Model selection*:: Use cheaper models for simple tasks. Haiku for grep/rename. Catches unnecessary expense.
endif::[]
...
*Month 4{plus}: The Factory*. Build MCP servers for project context. Create meta-tools (generators of generators). Enable self-improving infrastructure. Outcome: Exponential productivity.
...
[arabic]
. *Layer 1*: Is CLAUDE.md strong? Do hooks run?
. *Layer 2*: Does the repo have tests? OTEL? DDD structure?
. *Layer 3*: Are there automation scripts? Tests for tests?
. *Layer 4*: Are there constraints? Closed-loop optimization?
...
NOTE: *Companion Code*: All 5 code examples for this chapter are available at https://github.com/Just-Understanding-Data-Ltd/compound-engineering-book/tree/main/examples/ch13[examples/ch13/]

_Related chapters:_

* *<<_chapter_4_writing_your_first_claude_md,Chapter 4: Writing Your First CLAUDE.md>>* for CLAUDE.md fundamentals
* *<<_chapter_7_quality_gates_that_compound,Chapter 7: Quality Gates That Compound>>* for quality gates in depth
* *<<_chapter_9_context_engineering_deep_dive,Chapter 9: Context Engineering Deep Dive>>* for context engineering principles
* *<<_chapter_10_the_ralph_loop,Chapter 10: The RALPH Loop>>* for the RALPH loop in long-running work
* *<<_chapter_11_sub_agent_architecture,Chapter 11: Sub-Agent Architecture>>* for sub-agent architecture patterns
* *<<_chapter_16_building_autonomous_systems,Chapter 16: Building Autonomous Systems>>* for a concrete implementation of these principles
