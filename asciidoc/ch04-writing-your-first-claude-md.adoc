...
For content that fails the universal test, use progressive disclosure. Create an `agent++_++docs/` directory with focused files:
...
When working on `packages/workflows/src/send-email.ts`, Claude loads: - Root CLAUDE.md (40 lines) - workflows/CLAUDE.md (300 lines) - Total: 340 lines, 95%{plus} relevant
...
CLAUDE.md starts at 100 lines and grows to 5,000{plus} as teams add domain-specific patterns. Different developers working on different domains see irrelevant context that dilutes attention.
...
For the same Temporal workflow task, Claude now loads 45 {plus} 180 = 225 lines. Context relevance improved from 8% to 92%. First-try correctness improved from 35% to 78%. The team stopped correcting pattern mismatches.
...
[arabic]
. Count total lines
. Identify instructions that are not universally applicable
. Find any style guide rules that should use tooling instead
. Check for inline code snippets older than 6 months
. List what is missing from WHY, WHAT, or HOW
...
[arabic]
. Start with WHY: one paragraph on project purpose
. Add WHAT: tech stack and directory structure
. Add HOW: package manager, test commands, build commands
. Add conventions: 1-2 critical patterns maximum
...
. Estimate current lines per domain
. Sketch folder structure for domain files
. Design linking strategy between levels
. Calculate line counts before and after
. Identify which domain needs subdomain files
...
____
*Companion Code*: All 4 code examples for this chapter are available at https://github.com/Just-Understanding-Data-Ltd/compound-engineering-book/tree/main/examples/ch04[examples/ch04/]
____

_Related chapters:_ - link:ch02-getting-started-with-claude-code.md[Chapter 2: Getting Started with Claude Code] for installation and basic tool usage - link:ch03-prompting-fundamentals.md[Chapter 3: Prompting Fundamentals] for techniques that make your CLAUDE.md instructions more effective - link:ch09-context-engineering-deep-dive.md[Chapter 9: Context Engineering Deep Dive] for advanced information theory behind context optimization
