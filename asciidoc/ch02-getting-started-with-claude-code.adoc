...
Claude Code runs on macOS 11{plus}, Linux (Ubuntu 20{plus}), and Windows 10{plus} (with WSL2 (Windows Subsystem for Linux 2) or native). You need Node.js 18 or higher.
...
[arabic, start=4]
. Create a CLAUDE.md file in your project root. This file provides context about your project: what language you use, how to run tests, what patterns to follow. Claude Code reads it automatically at the start of every conversation.
...
==== Pattern 1: Context {plus} Goal {plus} Success Criteria
...
Why it works: Large requests generate 1,000{plus} lines where errors compound and debugging becomes archaeology. Incremental requests generate 20 to 100 lines at a time, where each piece can be validated immediately. Errors are caught at the source, not buried under subsequent code.
...
// PDF: Full comparison table
ifdef::backend-pdf[]
[width="99%",cols="28%,30%,20%,22%",options="header",]
|===
...
|Codebase context |Full repo via CLAUDE.md {plus} tools |Open files {plus} search |Paste-based, limited
...
// EPUB: Definition list for narrow screens
ifndef::backend-pdf[]
*Claude Code*:: Best for multi-file workflows and automation. Full repo context via CLAUDE.md. Speed: ~30s per turn. Runs tests, builds, linters. Token-based cost.

*Cursor*:: Best for quick edits and real-time coding. Context from open files plus search. Speed: <1s inline. Limited verification. Subscription cost.

*ChatGPT*:: Best for concepts and brainstorming. Paste-based, limited codebase context. Speed: ~5s API. No verification. Subscription cost.
endif::[]

*Use Claude Code when:* - Task spans 3{plus} files - You need to run tests or builds - Full codebase context is essential - You are building automation
...
[arabic]
. Ask Claude Code: "`What is the main entry point for this project?`"
. Ask: "`How does error handling work? Show me 3 examples.`"
. Ask: "`What testing framework is used? Show me one test file.`"
. Ask: "`What are the top 3 architectural patterns I should follow?`"
. Write a two-paragraph summary of what you learned.

Success criteria: You can name 3{plus} patterns, 2 design decisions, and you read actual code.

==== Exercise 2: Prompt Quality Comparison

[arabic]
. Write a vague prompt: "`Add a utility function.`"
. Rewrite it using Context {plus} Goal {plus} Success Criteria pattern.
. Run the rewritten prompt.
. Compare iterations needed.

Success criteria: Rewritten prompt is 3{plus} sentences, includes file paths, specifies success criteria, requires fewer iterations.

==== Exercise 3: Full Explore {plus} Implement Workflow
...
The next chapter covers prompting fundamentals in depth. You will learn information theory concepts that explain why some prompts work better than others, and you will develop techniques for maximizing the value per token. See <<_chapter_3_prompting_fundamentals,Chapter 3: Prompting Fundamentals>>.

'''''

NOTE: *Companion Code*: All 3 code examples for this chapter are available at https://github.com/Just-Understanding-Data-Ltd/compound-engineering-book/tree/main/examples/ch02[examples/ch02/]

_Related chapters:_ - <<_chapter_1_the_compound_systems_engineer,Chapter 1: The Compound Systems Engineer>> for the meta-engineering philosophy - <<_chapter_4_writing_your_first_claude_md,Chapter 4: Writing Your First CLAUDE.md>> for deep dives on project context files - <<_chapter_6_the_verification_ladder,Chapter 6: The Verification Ladder>> for test-driven agent workflows
