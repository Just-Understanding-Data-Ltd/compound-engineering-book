...
Real-world metrics from large codebases (50K{plus} lines of code) tell the story:
...
[[fig-context-hierarchy]]
.Context Hierarchy: How sub-agents inherit and specialize context
image::ch11-context-hierarchy.png[Context Hierarchy,width=600,align=center]
...
const agentPermissions = {
  backendEngineer: {
    canWrite: true,
    allowedPaths: ['packages/api/**', 'packages/domain/**'],  // <1>
    tools: ['Read', 'Write', 'Edit', 'Bash'],  // <2>
  },

  frontendEngineer: {
    canWrite: true,
    allowedPaths: ['packages/ui/**', 'apps/web/**'],
    tools: ['Read', 'Write', 'Edit'],  // <3>
  },

  qaEngineer: {
    canWrite: true,
    allowedPaths: ['**/*.test.ts', '**/*.spec.ts', 'tests/**'],  // <4>
...
[source,typescript]
----
// packages/api/src/routes/payments.ts  <1>
...
router.post(
  '/payments',
  authenticate,  // <2>
  validateSchema(createPaymentSchema),  // <3>
...
// packages/application/src/services/payment-service.ts  <4>
...
export async function createPayment(
  ...
  try {
    ...
    return {
      success: false,
      error: `Payment failed: ${error.message}`,  // <6>
    };
  }
}
----
...
*When Sub-Agents Win:* - High-stakes decisions (security review before deploy) - Complex analysis (entire codebase for performance issues) - Specialized domains (database optimization) - Large codebases (50K{plus} lines of code) - Production-critical code
...
. Critic reviews across 8 dimensions (security, architecture, performance, testing, error handling, documentation, accessibility, code quality)
. Critic reports findings with severity ratings
. Actor refactors to address issues
. Critic re-reviews
. Repeat until approved or maximum rounds reached (typically 3-5)
...
This workflow catches 90%{plus} of issues that would otherwise reach human review.
...
[arabic]
. Identify which specialized agents you need
. Write behavioral flows for two agents (100-200 lines each)
. Define tool access control for each agent
. Create orchestration flow (which agents run in parallel, which sequential)
...
[arabic]
. Write a critic prompt with the 8 critique dimensions
. Generate initial code (actor role)
. Review the code (critic role)
. Count issues found
. Refactor based on critique (actor role)
. Repeat until approved or 5 rounds reached
...
[arabic]
. Choose a simple update (dependency version, API change)
. Write explicit task description
. Spawn agents for 3-5 packages in parallel
. Verify changes are consistent across all packages
. Measure: Time taken versus estimated sequential time
...
NOTE: *Companion Code*: All 5 code examples for this chapter are available at https://github.com/Just-Understanding-Data-Ltd/compound-engineering-book/tree/main/examples/ch11[examples/ch11/]

_Related chapters:_

* *<<_chapter_4_writing_your_first_claude_md,Chapter 4: Writing Your First CLAUDE.md>>* for the foundation of three-layer context hierarchy
* *<<_chapter_6_the_verification_ladder,Chapter 6: The Verification Ladder>>* for the verification patterns sub-agents enforce
* *<<_chapter_7_quality_gates_that_compound,Chapter 7: Quality Gates That Compound>>* for how sub-agent output flows through quality gates
* *<<_chapter_10_the_ralph_loop,Chapter 10: The RALPH Loop>>* for how sub-agents integrate into the autonomous development cycle
* *<<_chapter_15_model_strategy_and_cost_optimization,Chapter 15: Model Strategy & Cost Optimization>>* for using different model tiers for different agent roles
