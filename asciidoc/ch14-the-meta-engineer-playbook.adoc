...
|Variable latency (Large Language Model (LLM) reasoning {plus} execution) |Fast, predictable execution
...
[[fig-leverage-stack]]
.The Leverage Stack: What to keep sharp vs what to safely delegate or forget
image::ch14-leverage-stack.png[Leverage Stack,width=600,align=center]
...
[arabic]
. Could I explain this without looking at it?
. Could I rewrite the core logic from memory?
. Could I reason about worst-case behavior?
...
[[fig-atrophy-ladder]]
.The Atrophy Ladder: Level 4 is the minimum for career safety
image::ch14-atrophy-ladder.png[Atrophy Ladder,width=600,align=center]
...
ifndef::backend-pdf[]
*Wave 1 - Traditional coding*:: Status: Declining. You type everything.

*Wave 2 - Code completions*:: Status: Declining. Copilot-style autocomplete.

*Wave 3 - Chat-based coding*:: Status: Current. Back-and-forth dialogue.

*Wave 4 - Coding agents*:: Status: Q1 2025. Autonomous multi-step execution.

*Wave 5 - Agent clusters*:: Status: Q2-Q3 2025. Parallel agents, human coordinates.

*Wave 6 - Agent fleets*:: Status: Early 2026. Supervisor agents manage pods.
endif::[]
...
[[fig-four-automation-levels]]
.The Four Levels of Automation: From 1x to 100-500x productivity
image::ch14-four-automation-levels.png[Four Automation Levels,width=600,align=center]
...
[arabic]
. Create `.claude/conversation-archive/` and copy your next three significant conversations there.
. Create a `.claude/commands/extract.md` prompt that extracts decisions, problems, solutions, and patterns from conversations.
. Create a spec template at `specs/features/template.md` with sections for requirements, edge cases, success criteria, and constraints.
...
[arabic]
. Could you explain the code without looking?
. Could you rewrite the core logic from memory?
. Could you reason about worst-case behavior?
. Could you defend the tradeoffs?
...
[arabic]
. *Convert repeated workflows to scripts.* If you have done it three times, make it deterministic. Scripts are for execution. Agents are for reasoning.
. *Treat prompts and specs as assets.* Code is derivative. Archive conversations, extract learnings, maintain specs as first-class artifacts.
. *Protect high-leverage skills.* Let syntax atrophy. Keep problem understanding, solution design, and verification sharp. Level 4 is the minimum for career safety.
. *Understand the waves.* We are transitioning from Wave 3 (chat) to Wave 4 (agents). Task decomposition and fleet coordination are the new skills.
. *Become a meta-builder.* Build the environments, constraints, and feedback loops that make good code inevitable. Systems that improve themselves are the goal.

The product is the output. The system is the asset. Build the factory.

'''''

NOTE: *Companion Code*: All 6 code examples for this chapter are available at https://github.com/Just-Understanding-Data-Ltd/compound-engineering-book/tree/main/examples/ch14[examples/ch14/]

_Related chapters:_

* *<<_chapter_1_the_compound_systems_engineer,Chapter 1: The Compound Systems Engineer>>* introduced the identity. This chapter operationalizes it.
* *<<_chapter_7_quality_gates_that_compound,Chapter 7: Quality Gates That Compound>>* covered quality gates. Meta-engineers automate gate enforcement.
* *<<_chapter_10_the_ralph_loop,Chapter 10: The RALPH Loop>>* covered the RALPH loop. Meta-engineers build infrastructure that makes it run autonomously.
* *<<_chapter_11_sub_agent_architecture,Chapter 11: Sub-Agent Architecture>>* covered sub-agents. Meta-engineers design the orchestration layer.
* *<<_chapter_13_building_the_harness,Chapter 13: Building the Harness>>* covered harness construction. This chapter explains why the harness matters.
* *<<_chapter_16_building_autonomous_systems,Chapter 16: Building Autonomous Systems>>* demonstrates meta-engineering with a real production case study.
