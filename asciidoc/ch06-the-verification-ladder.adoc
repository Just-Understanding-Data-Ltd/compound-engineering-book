...
.The Verification Ladder: Each level catches what lower levels miss
...
The tools mentioned here: Temporal Logic of Actions Plus (TLA{plus}) and Z3 are specification languages for formal proofs. fast-check (JavaScript) and Hypothesis (Python) are property-based testing frameworks. Zod and io-ts are TypeScript runtime validation libraries. mypy is Python’s static type checker.
...
const UserSchema = z.object({  // <1>
  id: z.string().uuid(),  // <2>
  email: z.string().email(),
  age: z.number().int().min(0).max(150),  // <3>
  role: z.enum(['admin', 'user', 'guest']),
});

// At your API boundary
app.post('/users', (req, res) => {
  const result = UserSchema.safeParse(req.body);  // <4>
  if (!result.success) {
    return res.status(400).json({ errors: result.error.issues });
  }
  // result.data is now typed AND validated
  createUser(result.data);  // <5>
});
----
...
ifndef::backend-pdf[]
*Unit tests*:: 47 tests needed, verifying 5-10 lines each. Signal per test: Low.

*Integration tests*:: 3 tests needed, verifying 50-100 lines each. Signal per test: High.
endif::[]
...
[[fig-test-pyramid-inversion]]
.Test Pyramid Inversion: LLM-optimized testing emphasizes integration tests
image::ch06-test-pyramid-inversion.png[Test Pyramid Inversion,width=600,align=center]
...
. *Commutativity*: `f(a, b) === f(b, a)` when order shouldn’t matter
...
|API endpoint |Level 2 (Schema) {plus} Level 4 (Integration)
|Financial calculations |Level 5 (Property tests)
|Security-critical code |Level 5 {plus} manual audit
...
[[fig-verification-sandwich]]
.The Verification Sandwich: Establish baseline before generation, detect delta after
image::ch06-verification-sandwich.png[Verification Sandwich,width=600,align=center]
...
*The multiplicative effect:* - Level 1-3 catches 80% of bugs - Level 5 catches 80% of the remaining 20% - Combined: 80% {plus} (80% × 20%) = 96% total catch rate
...
[arabic]
. *Agents can’t distinguish flaky from real failures.* They’ll try to "`fix`" code that isn’t broken.
. *Wasted API tokens.* The agent spends cycles analyzing false positives.
. *Context pollution.* Failed fix attempts add noise to conversation history.
. *Lost trust.* Developers stop trusting agent test feedback.
...
. Add Zod schema validation for the request body
...
[arabic]
. Create a verification script that runs type-check, lint, and tests
. Run it to establish baseline (should pass)
. Ask Claude to add a new feature
. Run verification again
. Identify which failures are from the new code vs. pre-existing
...
NOTE: *Companion Code*: All 3 code examples for this chapter are available at https://github.com/Just-Understanding-Data-Ltd/compound-engineering-book/tree/main/examples/ch06[examples/ch06/]

_Related chapters:_

* *<<_chapter_5_the_12_factor_agent,Chapter 5: The 12-Factor Agent>>* for the reliability principles that verification enforces
* *<<_chapter_7_quality_gates_that_compound,Chapter 7: Quality Gates That Compound>>* for building verification systems that improve over time
* *<<_chapter_3_prompting_fundamentals,Chapter 3: Prompting Fundamentals>>* for structuring prompts that produce testable code
