...
### Supplementary Sources (Added from KB Analysis Jan 28, 2026)
5. **agent-memory-patterns.md** - Three-tier memory hierarchy (session, file-based, event-sourced), checkpoint/resume patterns for RALPH compatibility, webhook integration for human approval
6. **checkpoint-commit-patterns.md** - Git strategies for AI-assisted development, commit timing patterns, recovery patterns
7. **orchestration-patterns.md** - Coordinator, Swarm, and Pipeline patterns for multi-agent RALPH workflows
8. **12-factor-agents.md** - Factor 6 (launch/pause/resume), Factor 10 (small focused agents), Factor 12 (stateless reducer) directly applicable to RALPH iteration design
