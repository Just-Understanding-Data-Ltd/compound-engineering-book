...
### New Sources (Added from KB Analysis Jan 28, 2026)
8. **zero-friction-onboarding.md** - Critical for installation/setup section. Key insight: "Setup time from git clone to working system directly correlates with AI coding agent effectiveness. If a junior developer can't get the system running in 5 minutes, Claude Code will struggle too." Recommends automated validation scripts for prerequisites, environment variables, and service health.

9. **human-first-dx-philosophy.md** - Foundational for why CLAUDE.md and good DX matter. Key insight: "If your repository is easier to create, edit, build and test for humans, that translates directly to LLM performance. The correlation is approximately 0.85." Supports the chapter's focus on documentation and clear project structure.

10. **incremental-development-pattern.md** - Essential for prompting patterns section. Key insight: "Large feature requests to AI coding agents lead to errors buried in 1000+ lines of generated code. Breaking work into smallest possible increments reduces error rates by 90%." Provides concrete before/after examples and the validation loop pattern.

11. **writing-a-good-claude-md.md** - Critical update for CLAUDE.md guidance. Key insights:
    - Claude may ignore CLAUDE.md if instructions aren't universally applicable (Anthropic's system prompt tells it to filter)
    - Research shows ~150-200 instruction limit before performance degrades
    - Claude Code's system prompt already contains ~50 instructions (uses 1/3 of capacity)
    - "Less is more" - optimal length < 300 lines, ideally < 100
    - Progressive disclosure pattern: reference task-specific docs, don't embed everything
    - Never use Claude as a linter (delegate to deterministic tools)

12. **trust-but-verify-protocol.md** - Enhances Pattern 4 (Tests as Specification). Key insight: "Don't trust AI output - ask AI to create verification instead. Instead of reviewing 1000+ lines of code, review 10 lines of test output. Reduces review burden by 99%, catches bugs at generation time."

13. **six-waves-of-ai-coding.md** - Industry context for why Claude Code matters. Key insight: Wave 4 (coding agents operating autonomously) requires task decomposition skill. "Oversizing tasks causes agents to fail spectacularly." Helps readers understand where Claude Code fits in the evolution of AI-assisted development.
...
5. First command: `claude -p "List the main files in src/"`
...
**Subsections:**
- **Starting a Conversation**: `claude -p "<prompt>"` for single queries vs `claude` (interactive mode).
...
```bash
claude -p "Show me the current project structure. What files exist in src/?"
```
...
```bash
claude -p "Create a simple CLI tool that reads a CSV file and prints a summary.
...
```bash
claude -p "The CSV reader is good, but add support for filtering by column name.
...
**Example**:
```bash
claude -p "Run npm test to verify the payment tests pass"
```
...
claude -p "Should I use Result<T, E> or throw exceptions for the new payment service?"
...
```bash
claude -p "Create PaymentService following these patterns from exploration:
...
```bash
# Question 1
claude -p "Do we have existing rate limiting? Show me the implementation."
# Answer: RateLimiter in src/middleware/rateLimit.ts

# Question 2
claude -p "What endpoints currently use rate limiting?"
# Answer: /api/login, /api/signup. See POST /users endpoint for pattern.

# Question 3
claude -p "What's our error response format for rate limit exceeded?"
# Answer: { success: false, error: { code: 'RATE_LIMIT_EXCEEDED', ... } }

# Question 4
claude -p "What configuration keys should I use?"
# Answer: window: 60s, max: 100 requests, stored in config/rateLimit.ts
```

Implementation (2 min):
```bash
claude -p "Add rate limiting to POST /api/data endpoint.
...
### 4.8 Common Pitfalls for Newcomers (NEW SECTION - 500 words)

**Subsections:**
- **Pitfall 1: Oversized Tasks**
- **Pitfall 2: Skipping Exploration**
- **Pitfall 3: Long Conversations**
- **Pitfall 4: Bloated CLAUDE.md**
- **Pitfall 5: Manual Review of Generated Code**

Based on KB articles: incremental-development-pattern.md, writing-a-good-claude-md.md, cursor-agent-workflows.md, trust-but-verify-protocol.md

#### 4.8.1 Pitfall 1: Oversized Tasks

**The Mistake**: Asking Claude Code to build complete features in single requests.

```
BAD: "Build a complete authentication system with JWT tokens, password hashing,
login/logout endpoints, password reset flow, rate limiting, and admin dashboard."
```

**The Problem**: 1000+ lines generated, errors buried in volume, 65% chance of bugs (probability compounds per 100 lines).

**The Fix**: Break into smallest possible increments. Each increment: 20-100 lines, validated immediately.

```
BETTER:
1. "Create User interface with id, email, passwordHash"
2. "Add hashPassword and verifyPassword functions using bcrypt"
3. "Create authenticate function returning AuthResult"
(Continue incrementally, validating each step)
```

**Metric**: 90% error rate reduction through incremental development (source: incremental-development-pattern.md).

#### 4.8.2 Pitfall 2: Skipping Exploration

Already covered in 4.7 but worth reiterating:

**The Mistake**: Jumping straight to implementation without understanding patterns.

**Metric**: 60% fewer iterations when exploration precedes implementation (source: llm-usage-modes-explore-vs-implement.md).

#### 4.8.3 Pitfall 3: Long Conversations

**The Mistake**: Running 20+ turn conversations thinking more context helps.

**The Problem**: "Long conversations cause agents to lose focus. After many turns and summarizations, context accumulates noise." (source: cursor-agent-workflows.md)

**The Fix**: Start fresh when shifting tasks, completing logical units, or when agent becomes confused/repetitive.

#### 4.8.4 Pitfall 4: Bloated CLAUDE.md

**The Mistake**: Dumping entire style guides, API docs, and database schemas into CLAUDE.md.

**The Problem**: Research shows ~150-200 instruction limit. Claude Code's system prompt uses ~50. Your CLAUDE.md competes for remaining capacity.

**The Fix**:
- Keep CLAUDE.md < 100 lines (universally applicable guidance only)
- Use progressive disclosure: `"For database work, see docs/database.md"`
- Never include linting rules (use actual linters)
- Reference files, don't embed code snippets (they go stale)

(source: writing-a-good-claude-md.md)

#### 4.8.5 Pitfall 5: Manual Review of Generated Code

**The Mistake**: Reading every line of 1000-line generated feature trying to spot bugs.

**The Problem**: Human review catches 40-60% of bugs, takes hours, creates false confidence.

**The Fix**: "Don't trust AI output - ask AI to create verification instead."

```
BETTER: "Implement the feature, then create verification tests that check:
- Happy path
- All error cases
- Edge cases
...
**Metric**: Review test output (10 lines) instead of code (1000 lines). 99% reduction in review time, 2x better bug detection (source: trust-but-verify-protocol.md).

---

### 4.9 Exercises: Try It Yourself (400 words)
...
#### 4.9.1 Exercise 1: Exploration Practice (15 min)
...
#### 4.9.2 Exercise 2: Basic Prompting (20 min)
...
#### 4.9.3 Exercise 3: Full Explore + Implement Workflow (30 min)
...
| 4.8 Common Pitfalls (NEW) | 500 |
| 4.9 Exercises | 400 |
| Sidebars & Examples | 500 |
| **Total Chapter** | **6,300 words** |

**Rationale**: Beginner-friendly length (18-22 minute read). Heavy on examples and walkthroughs. All patterns are immediately applicable. Common Pitfalls section added to prevent newcomer mistakes based on KB research.
