...
```
Start: New code → Is it compiling? (Implies Level 1 ✓)
  ↓
  Is it handling external data? → Yes → Add Level 2 (Schema) ✓
  ↓
  Is it business logic? → Yes → Add Level 3 (Unit tests) ✓
  ↓
  Does it interact with other components? → Yes → Add Level 4 (Integration) ✓
  ↓
  Is it calculating money/security? → Yes → Add Level 5 (Property tests) ✓
  ↓
  Is it distributed consensus/life-critical? → Yes → Add Level 6 (Formal) ✓
  ↓
...
- Information Theory: Constraining the Solution Space
  - Formula: S_constrained = S ∩ T₁ ∩ T₂ ∩ ... ∩ Tₙ ≈ C
...
npm test
// ✗ validatePassword() is not defined
...
- The Solution: Ask AI to Generate Verification
  - Instead of: AI writes code → You review everything
  - Do this: AI writes code → AI writes verification → You review verification
...
- Compound Learning: Each Iteration Teaches the AI
  - Iteration 1: Missing rate limiting → Test fails → AI learns
  - Iteration 2: Includes rate limiting → Test passes → Confirmed
...
3. **Unit vs Integration Tests**
   ...
   - Integration test: API call → database → API response
...
4. **Property Test Discovering a Bug**
   ...
   - Shrinking shows minimal failing case: `"pass😀"`
...
- Three phases: Pre-Verification → Generation → Post-Verification
...
- Show success rate improving: 0.001% → 60%

### 6.5 Test-Driven Prompting Workflow
- Flowchart: Write Tests → Verify Fail → Prompt LLM → Run Tests → Iterate
- Decision diamond: "All tests pass?" → Yes/No branches
- Feedback loop showing iteration

### 6.6 Quality Gate Stack in CI/CD
- Vertical stack showing order: Type-check → Lint → Unit Tests → Integration → Property Tests → (optional Formal)
...
- Cycle: Generate → Verify → Learn → Generate Better
...
```typescript
// Write full API flow: POST → database → response
...
```bash
# Pre-verification: Run all tests → all pass
# Generation: Have LLM add password reset feature
# Post-verification: Run all tests again → identify new failures
```
...
│  ├─ AI generates syntactically correct but behaviorally wrong code
│  ├─ Manual review doesn't scale
│  └─ Each level catches what lower levels miss
│
├─ 4.2 Level 1: Static Types
│  ├─ What it catches
│  ├─ What it misses
│  └─ When to use
│
├─ 4.3 Level 2: Runtime Validation
│  ├─ The boundary problem
│  ├─ Zod examples
│  └─ Error handling patterns
│
├─ 4.4 Level 3: Unit Tests
│  ├─ Logic verification in isolation
│  ├─ What it catches vs misses
│  └─ Best practices
│
├─ 4.5 Level 4: Integration Tests
│  ├─ Component interactions
│  ├─ Real vs mocked dependencies
│  └─ Cost vs confidence
│
├─ 4.6 Level 5: Property-Based Testing
│  ├─ Automatic edge case discovery
│  ├─ fast-check & Hypothesis
│  ├─ Patterns for LLM code
│  └─ Test case shrinking
│
├─ 4.7 Level 6: Formal Verification
│  ├─ TLA+ for distributed systems
...
│  └─ Real-world examples
│
├─ 4.9 Verification Sandwich Pattern
│  ├─ Pre-verification baseline
│  ├─ Generation phase
│  ├─ Post-verification delta
│  └─ Key rule: Failures are blockers
│
├─ 4.10 Test-Driven Prompting
│  ├─ Entropy reduction
│  ├─ Information theory foundation
│  ├─ Workflow
│  └─ 3-7x improvement in success rate
│
├─ 4.11 Property-Based Testing Deep Dive
│  ├─ Why LLMs struggle with edge cases
│  ├─ Converting examples to properties
│  ├─ Common patterns
│  ├─ Test case shrinking
│  └─ Integration with test-driven prompting
│
├─ 4.12 Trust But Verify Protocol
│  ├─ Why manual review doesn't scale
│  ├─ AI-generated verification artifacts
│  ├─ Types: runtime, visual, data, API
│  ├─ Workflow
│  └─ Compound learning
│
├─ 4.13 Compound Quality Gate Systems
│  ├─ Layering for multiplicative impact
│  ├─ CI/CD implementation
│  ├─ Feedback loops
│  └─ Measuring impact
│
├─ 4.14 Case Study: E-Commerce Checkout
│  ├─ Feature overview
...
│  ├─ Wrong level for risk
│  ├─ Skipping pre-verification
│  ├─ Vague or over-specific tests
│  ├─ Not running verification
│  ├─ Partial coverage
│  └─ Non-deterministic tests
│
└─ 4.16 Integration with Other Practices
