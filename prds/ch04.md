...
4. **agent-sdk-patterns.md** - Agent SDK v2-preview API patterns (unstable_v2_* functions)
5. **human-in-the-loop-patterns.md** (TO BE CREATED) - Detailed HITL implementation patterns
6. **event-sourcing-agents.md** (TO BE CREATED) - Event log architecture for agent state
7. **tool-call-validation.md** (TO BE CREATED) - JSON schema validation for tool outputs
...
for await (const msg of session.stream()) {
  ...
  if (text) {
    deploymentPlan = text
    console.log('📋 Deployment Plan:', text)
  }
}

// Human approval gate
console.log('\n⚠️  HIGH RISK ACTION: Production Deployment')
...
if (approval.toLowerCase() === 'y') {
  ...
  for await (const msg of session.stream()) {
    const text = getAssistantText(msg)
    if (text) console.log('✅ Executing:', text)
  }
} else {
  ...
  for await (const msg of session.stream()) {
    const text = getAssistantText(msg)
    if (text) console.log('🛑 Alternatives:', text)
  }
}
```
...
for await (const msg of session.stream()) {
  ...
  if (text) {
    plan = text
    console.log('📋 Email Plan:', text)
  }
}

// Human approval gate
console.log('\n⚠️  This will send 5000 emails')
...
if (approval.toLowerCase() === 'y') {
  ...
  for await (const msg of session.stream()) {
    const text = getAssistantText(msg)
    if (text) console.log('📝 Saved:', text)
  }
}
```
