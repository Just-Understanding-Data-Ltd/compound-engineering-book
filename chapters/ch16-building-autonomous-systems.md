...
The review interval adapts based on issue counts. The algorithm uses a convergence-based approach similar to TCP congestion control:
...
EPUB formatting is invisible to text-only agents. An agent can read the CSS file and the markdown source, but it cannot see how the book actually renders in a reader application.
...
For this book:
- Queue update script: 2 hours to build, 5 minutes per use, 170+ uses = 14 hours saved
- EPUB review skill: 4 hours to build, 30 minutes per review, 20+ reviews = 6 hours saved
- Slop-checker agent: 1 hour to build, 10 minutes per review, 30+ reviews = 4 hours saved
...
Total infrastructure investment: approximately 17 hours.
Total time saved: approximately 38 hours.
Return on investment: 2.2x.

The ROI calculation underestimates the real value. Infrastructure remains available for future projects. The next book starts at phase 3 velocity, not phase 1.
...
*Related chapters:*
- [Chapter 10: The RALPH Loop](ch10-the-ralph-loop.md) for loop fundamentals
- [Chapter 11: Sub-Agent Architecture](ch11-sub-agent-architecture.md) for agent orchestration
- [Chapter 13: Building the Harness](ch13-building-the-harness.md) for infrastructure layers
